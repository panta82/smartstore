...
function SmartStore(options) {
	...
	function openSync() {
		if (_opening) {
			...
			throw new SmartStoreError(`Store is already loading async (call either load or loadSync, not both)`);
		}
	...
	function closeSync() {
		if (_closing) {
			throw new SmartStoreError(`Store is already closing async (call either close or closeSync, not both)`);
		}

		if (_opening) {
			throw new SmartStoreError(`Store is currently being opened async (call either sync or async methods, not both)`);
		}
	...
	function triggerAutoFlush() {
		const now = new Date();
		if (_flushWait && now - _flushWait.startedAt >= options.auto_flush_delay[1] - options.auto_flush_delay[0]) {
			// No more delays
			return;
		}
		...
		_flushWait.timeout = setTimeout(doFlush, options.auto_flush_delay[0]);
	}
	...
	function doFlush(sync = false) {
		...
		if (!dirtyCount) {
			...
			return _flushing || Promise.resolve();
		}

		log && log(`Found ${dirtyCount} dirty keys. Initiating write`);
